/*
* Copyright (c) 2014 WyzePal, Inc
*/

package org.wyzepal.jira
...
class WyzePalListener extends AbstractIssueEventListener {
    Logger LOGGER = Logger.getLogger(WyzePalListener.class.getName());

    // The email address of one of the bots you created on your WyzePal settings page.
    String wyzepalEmail = ""
    // That bot's API key.
    String wyzepalAPIKey = ""

    // What stream to send messages to. Must already exist.
    String wyzepalStream = "jira"

    // The base JIRA url for browsing
    String issueBaseUrl = "https://jira.COMPANY.com/browse/"

    // Your wyzepal domain
    String base_url = "https://wyzepal.example.com/"
    ...
    String post(String method, NameValuePair[] parameters) {
      ...
      try {
        ...
        if (post.getStatusCode() != HttpStatus.SC_OK) {
          ...
          LOGGER.log(Level.SEVERE, "Error sending WyzePal message:\n" + response + "\n\n" +
                                   "We sent:" + params)
        }
        return response;
      } catch (IOException e) {
    ...
    String sendStreamMessage(String stream, String subject, String message) {
      NameValuePair[] body = [new NameValuePair("api-key", wyzepalAPIKey),
                              new NameValuePair("email",   wyzepalEmail),
