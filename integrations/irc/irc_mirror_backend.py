...
def wyzepal_sender(sender_string):
    # type: (str) -> str
    nick = sender_string.split("!")[0]
    return nick + "@" + IRC_DOMAIN

class IRCBot(irc.bot.SingleServerIRCBot):
    def __init__(self, wyzepal_client, channel, nickname, server, port=6667):
        # type: (Any, irc.bot.Channel, str, str, int) -> None
        irc.bot.SingleServerIRCBot.__init__(self, [(server, port)], nickname, nickname)
        self.channel = channel  # type: irc.bot.Channel
        self.wyzepal_client = wyzepal_client

    def on_nicknameinuse(self, c, e):
        # type: (ServerConnection, Event) -> None
        c.nick(c.get_nickname().replace("_wyzepal", "__wyzepal"))
    ...
    def on_welcome(self, c, e):
        ...
        ## Forwarding from WyzePal => IRC is disabled; uncomment the next
        ## line to make this bot forward in that direction instead.
        #
        # self.wyzepal_client.call_on_each_message(forward_to_irc)

    def on_privmsg(self, c, e):
        # type: (ServerConnection, Event) -> None
        content = e.arguments[0]
        sender = wyzepal_sender(e.source)
        if sender.endswith("_wyzepal@" + IRC_DOMAIN):
            return

        # Forward the PM to WyzePal
        print(self.wyzepal_client.send_message({
            "sender": sender,
            "type": "private",
    ...
    def on_pubmsg(self, c, e):
        ...
        # Forward the stream message to WyzePal
        print(self.wyzepal_client.send_message({
            "forged": "yes",
            "sender": sender,
            "type": "stream",
